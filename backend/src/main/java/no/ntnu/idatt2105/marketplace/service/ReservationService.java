...
public class ReservationService {
  ...
  /**
   * Retrieves a list of items reserved by a specific user.
   *
   * @param userId the unique identifier of the user
   * @return a list of ItemResponseDTO objects representing the reserved items
   */
  public List<ItemResponseDTO> getReservedItemsByUserId(Long userId) {
    List<Reservation> reservations = reservationRepository.findByUserId(userId);
    return reservations.stream()
        .map(reservation -> itemService.convertToResponse(reservation.getItem()))
        .toList();
  }
   ...
   */
  public void saveReservation(Long userId, Long itemId) {
    // Fetch user and item
